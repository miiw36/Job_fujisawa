...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // ユーザー名のバリデーション
    if (empty($username)) {
        $error_message .= "ユーザー名を入力してください。<br>";
    }
    ...
    if (empty($email)) {
        ...
        $error_message .= "Gmailのメールアドレスを入力してください。<br>";
    }
    ...
    if (empty($password)) {
        ...
        $error_message .= "パスワードは少なくとも4文字以上で、大文字、小文字、数字、特殊文字を1つ以上含む必要があります。<br>";
    }
...
<div class="container">
    ...
    <p>New user? <a href="register.php">新規の方はこちらで登録してください</a></p>
</div>
...
function validateEmail() {
    ...
    if (!emailPattern.test(emailInput.value)) {
        emailError.textContent = '有効なGmailアドレスを入力してください。';
    } else {
...
function validatePassword() {
        ...
        if (password.length < 4 || !/[A-Z]+/.test(password) || !/[a-z]+/.test(password) || !/[0-9]+/.test(password) || !/[!@#$%^&*()\-_=+{};:,<.>ยง?]+/.test(password)) {
            passwordError.textContent = 'パスワードは少なくとも4文字以上で、大文字、小文字、数字、特殊文字を1つ以上含む必要があります。';
        } else {
